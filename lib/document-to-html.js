...
function DocumentConversion(options, comments) {
    ...
    // Helper function to extract plain text from an element
    ...
    function convertParagraph(element, messages, options) {
        ...
        // Extract tagName from path structure: path._elements[0].tagName
        var tagName = path && path._elements && path._elements[0] && path._elements[0].tagName;
        var isHeading = tagName && /^h[1-6]$/i.test(tagName);
        
        // Check if this heading is a bibliography section heading by its text content
        var isBibliographyHeading = false;
        if (isHeading && element.children) {
            var text = extractTextFromElement(element);
            isBibliographyHeading = /^(Bibliography|References|Works\s+Cited)$/i.test(text.trim());
        }
        
        // Handle bibliography section headings specially
        if (isBibliographyHeading) {
            // Mark bibliography section headings with special class
            path = htmlPaths.element(tagName, {"class": "docx-bibliography-header"}, {fresh: true});
            // Pass context to children to skip citation wrapping
            var extendedOptions = Object.assign({}, options, {_inBibliographySection: true});
        ...
        // Add special node markers for bibliography section content
        if (element._bibliographySection) {
            // Wrap with bibliography marker class for easy detection by web app
    ...
    function convertRun(run, messages, options) {
        ...
        if (run._citationMetadata && !options._inBibliographySection) {
            // Wrap in a span with citation data attributes
            var citationAttrs = {
                "class": "docx-citation",
                "data-citation": JSON.stringify(run._citationMetadata)
            };
            paths.push(htmlPaths.element("span", citationAttrs, {fresh: false}));
        }
    ...
    var elementConverters = {
        ...
        "math": function(element, messages, options) {
            // Convert Office Math to a marked span for web module processing
            var attributes = {
                "class": "docx-math",
                "data-math-text": element.mathText
            };
            return [Html.nonFreshElement("span", attributes, [Html.text(element.mathText)])];
        },
