...
function DocumentConversion(options, comments) {
    ...
    function convertParagraph(element, messages, options) {
        ...
        // Add special node markers as CSS classes
        if (element._bibliographySection) {
            // Check if this is a heading style (h1-h6) by examining the path's tagName
            var isHeading = path && path.tagName && /^h[1-6]$/i.test(path.tagName);
            
            if (isHeading) {
                // Mark bibliography section headings with special class
                path = htmlPaths.element(path.tagName, {"class": "docx-bibliography-header"}, {fresh: true});
            } else {
