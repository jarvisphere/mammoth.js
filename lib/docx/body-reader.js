...
var citationDetector = require("./citation-detector");
...
function BodyReader(options) {
    ...
    var bibliography = options.bibliography || {};
    ...
    function readFldChar(element) {
        ...
        if (type === "begin") {
            ...
            if (complexFieldEnd.type === "citation") {
                // Mark this as a citation for later processing
                complexFieldStack.push(complexFieldEnd);
            }
            // Bibliography field ended - already popped from stack above
            // No need to push it back since the field is closing
    ...
    function isInBibliographyField() {
        return complexFieldStack.some(function(complexField) {
            return complexField.type === "bibliography";
        });
    }
    ...
    function parseInstrText(instrText, fldChar) {
        ...
        // Check for citation/bibliography fields
        var citationField = citationDetector.parseCitationInstrText(instrText);
        if (citationField) {
            return {type: "citation", citationData: citationField};
        }

        var bibliographyField = citationDetector.parseBibliographyInstrText(instrText);
    ...
    var xmlElementReaders = {
        ...
        "m:oMathPara": function(element) {
            // Math paragraph - contains m:oMath elements
            return readXmlElements(element.children);
        },
        "m:oMath": function(element) {
            // Office Math element
            var mathText = mathReader.extractMathText(element);
            return elementResult(new documents.Math(mathText));
        },
        ...
        "w:sdt": function(element) {
            // Check SDT type BEFORE reading content to set context properly
            var isBibliography = citationDetector.isBibliographySdt(element);
            var isCitation = citationDetector.isCitationSdt(element);
            
            // If entering a bibliography SDT, increment depth
            if (isBibliography) {
                bibliographySdtDepth++;
            }
            
            // Read the SDT content with the proper context
            var contentResult = readXmlElements(element.firstOrEmpty("w:sdtContent").children);
            
            // If exiting a bibliography SDT, decrement depth
            if (isBibliography) {
                bibliographySdtDepth--;
            }
            
            return contentResult.map(function(content) {
                // Mark bibliography section paragraphs for easy detection by web app
                if (isBibliography) {
                ...
                // (bibliography entries themselves are formatted as citation SDTs in Word)
                if (isCitation && bibliographySdtDepth === 0 && !isInBibliographyField()) {
                    var citationMetadata = citationDetector.extractCitationMetadata(element);

                    // Check if there's a citation field code on the stack
                    var citationField = _.last(complexFieldStack.filter(function(field) {
                        return field.type === "citation";
                    }));

                    // Merge field code data with SDT metadata for complete citation info
                    if (citationField && citationField.citationData) {
                        citationMetadata.tag = citationField.citationData.tag;
                        citationMetadata.fieldArgs = citationField.citationData.arguments;
                        citationMetadata.rawInstrText = citationField.citationData.rawInstrText;

                        // Look up full citation data in bibliography using the tag
                        var citationKey = citationField.citationData.tag;
                        if (citationKey && bibliography[citationKey]) {
                            citationMetadata.bibliographyData = bibliography[citationKey];
                        }
                    }

                    // Mark ALL run elements with citation metadata recursively
                    return content.map(transforms._elementsOfType(
                        documents.types.run,
                        function(run) {
                            run._citationMetadata = citationMetadata;
                            return run;
                        }
                    ));
                }
