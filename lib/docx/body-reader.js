            ...
            return contentResult.map(function(content) {
                ...
                if (citationDetector.isCitationSdt(element)) {
                    ...
                    if (citationField && citationField.citationData) {
                        citationMetadata.fieldCode = citationField.citationData.fieldCode;
                        ...
                        // Extract citation key from field arguments (e.g., "WuJ24" from "WuJ24 \l 1033")
                        var citationKey = citationField.citationData.arguments.split(/\s+/)[0];
                        
                        // Look up full citation data in bibliography
                        if (bibliography[citationKey]) {
                            citationMetadata.bibliographyData = bibliography[citationKey];
                        }
                    }
                    
                    // Mark ALL content items with citation metadata recursively
                    function markWithCitationMetadata(items) {
                        return items.map(function(item) {
                            if (item.type === documents.types.run) {
                                item._citationMetadata = citationMetadata;
                            } else if (item.type === documents.types.paragraph && item.children) {
                                item.children = markWithCitationMetadata(item.children);
                            }
                            return item;
                        });
                    }
                    
                    var markedContent = markWithCitationMetadata(content);
